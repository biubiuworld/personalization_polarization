...
class ExperimentPayment(Page):
    ...
    def vars_for_template(self):
        return {
            ...
            'payoff_experiment_dollar': round(
                float(self.participant.vars['VlFmH0_game_payoff'] + self.participant.vars['VhFhH0_game_payoff']) * 0.01,
                2),
    ...
    def before_next_page(self):
        self.player.total_payoff_experiment_dollar = round(round(float(self.participant.vars['VlFmH0_game_payoff']+self.participant.vars['VhFhH0_game_payoff'])*0.01,2)+Constants.participation_fee+Constants.endowment,2)
