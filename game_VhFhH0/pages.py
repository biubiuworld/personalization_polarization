...
class GameInstruction(Page):
    ...
    def before_next_page(self):
        ...
        self.participant.vars['VhFhH0_game_payoff'] = 0.
...
class OpinionUpdate(Page):
    ...
    def before_next_page(self):
        ...
        self.participant.vars['VhFhH0_game_payoff'] = self.player.payoff
        self.player.game_payoff = self.participant.vars['VhFhH0_game_payoff']
class Results(Page):
    # def is_displayed(self):
...
class GamePayment(Page):
    ...
    def vars_for_template(self):
        return{
            'VhFhH0_game_payoff': self.participant.vars['VhFhH0_game_payoff'],
        }
...
class ExperimentPayment(Page):
    ...
    def vars_for_template(self):
        return {
            ...
            'payoff_experiment': self.participant.vars['VlFmH0_game_payoff']
                                 + self.participant.vars['VhFhH0_game_payoff'],
            'payoff_experiment_dollar': round(
                float(self.participant.vars['VlFmH0_game_payoff'] + self.participant.vars['VhFhH0_game_payoff']) * Constants.dollar_per_credit,
                2),
    ...
    def before_next_page(self):
        self.player.total_payoff_experiment_dollar = round(round(float(self.participant.vars['VlFmH0_game_payoff']+self.participant.vars['VhFhH0_game_payoff'])*Constants.dollar_per_credit,2)+Constants.participation_fee+Constants.endowment,2)
