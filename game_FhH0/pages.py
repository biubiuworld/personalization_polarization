...
class GameInstruction(Page):
    ...
    def before_next_page(self):
        ...
        self.participant.vars['FhH0_payoff_in_all_rounds'] = []
...
class NeighborUpdate(Page):
    ...
    def before_next_page(self):
        ...
        self.participant.vars['FhH0_payoff_in_all_rounds'].append(self.player.payoff)
...
class GamePayment(Page):
    ...
    def vars_for_template(self):
        last_rounds = 5
        game_payoff_selection_list = self.participant.vars['FhH0_payoff_in_all_rounds'][-last_rounds:]
        self.participant.vars['FhH0_game_payoff'] = max(random.choice(game_payoff_selection_list), 0)
        self.player.game_payoff = self.participant.vars['FhH0_game_payoff']
        return{
            'FhH0_game_payoff': self.participant.vars['FhH0_game_payoff'],
        }
