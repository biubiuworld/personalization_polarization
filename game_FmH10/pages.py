...
class GameInstruction(Page):
    ...
    def before_next_page(self):
        ...
        self.participant.vars['FmH10_game_payoff'] = 0.
        self.participant.vars['FmH10_payoff_in_all_rounds'] = []
...
class NeighborUpdate(Page):
    ...
    def before_next_page(self):
        ...
        self.participant.vars['FmH10_payoff_in_all_rounds'].append(self.player.payoff)
...
class GamePayment(Page):
    ...
    def vars_for_template(self):
        last_rounds = 5
        game_payoff_selection_list = self.participant.vars['FmH10_payoff_in_all_rounds'][-last_rounds:]
        self.participant.vars['FmH10_game_payoff'] = max(random.choice(game_payoff_selection_list), 0)
        self.player.game_payoff = self.participant.vars['FmH10_game_payoff']
        return{
            'FmH10_game_payoff': self.participant.vars['FmH10_game_payoff'],
        }
...
class ExperimentPayment(Page):
    ...
    def vars_for_template(self):
        return {
            'payoff_game1': self.participant.vars['FhH10_game_payoff'],
            'payoff_game2': self.participant.vars['FlH10_game_payoff'],
            'payoff_game3': self.participant.vars['FmH10_game_payoff'],
            'payoff_experiment': self.participant.vars['FmH10_game_payoff']
                                 + self.participant.vars['FhH10_game_payoff']+ self.participant.vars['FlH10_game_payoff'],
            'payoff_experiment_dollar': round(
                float(self.participant.vars['FmH10_game_payoff'] + self.participant.vars['FhH10_game_payoff']
                      + self.participant.vars['FlH10_game_payoff']) * Constants.dollar_per_credit,2),
            'total_endowments': Constants.endowment*3,
            'total_payoff_experiment_dollar': round(round(
                float(self.participant.vars['FmH10_game_payoff'] + self.participant.vars['FhH10_game_payoff']
                      + self.participant.vars['FlH10_game_payoff']) * Constants.dollar_per_credit,
                2) + Constants.participation_fee + Constants.endowment*3, 2),
        }

    def before_next_page(self):
        self.player.total_payoff_experiment_dollar = round(round(float(self.participant.vars['FmH10_game_payoff'] + self.participant.vars['FhH10_game_payoff']
                      + self.participant.vars['FlH10_game_payoff'])*Constants.dollar_per_credit,2)+Constants.participation_fee+Constants.endowment*3,2)
